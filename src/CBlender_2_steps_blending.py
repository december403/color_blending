...
class CBlender():
    def __init__(self,tar_img, ref_img, seam_mask, tar_4_corners, ref_4_corners, homoMat, shiftMat, mask):
        ...
        self.mask = mask
        # self.devided_tar_nonoverlap_mask, self.devided_tar_edge_mask = self.__split_tar_nonoverlap_area_and_edge(ref_4_corners, tar_4_corners, shiftMat, homoMat)
    ...
    def  __calc_color_compensation(self, sampling_pixels_coordi_lst, refered_pixel_coordi_lst, sigma1=0.5, sigma2=0.5):
        ...
        color_compensation_lst = numba_calc_color_compensation( color_compensation, color_compensation_lst, warp_tar_img_norm,\
             refered_pixel_coordi_lst, seam_color_diff_lst, sampling_pixels_coordi_lst, M, sigma1=sigma1, sigma2=sigma2 )
...
refered_pixel_coordi_lst = np.array( np.where(devided_tar_edge_mask==255) ).T
...
CB.ref_img = fuck_this_shit
refered_pixel_coordi_lst = np.array( np.where(devided_tar_edge_mask==127) ).T
mask = np.zeros(devided_tar_nonoverlap_mask.shape)
mask[devided_tar_nonoverlap_mask==127] = 255
fuck_this_shit  = CB.blend_color(refered_pixel_coordi_lst, mask, sigma1=0.5, sigma2=0.05)
